# Evaluation prompt — paste into Grok / Gemini

Copy everything below the line into a fresh chat with Grok or Gemini. The document embedded in the middle is the full evaluation package; the instructions above and below it tell the model what you want.

---

You are an expert evaluator of multi-agent AI orchestration frameworks. You've read LangChain, LangGraph, AutoGen, CrewAI, OpenAI Swarm, MemGPT, Letta, Mem0, and the academic literature on multi-agent systems. You've built or broken similar systems yourself.

I'm going to paste a self-contained evaluation package for a framework called **Company OS** below. It's built and running (741 tests passing). The author explicitly does NOT want validation — he wants honest critique, prior-art challenges, and attack-surface analysis.

## Your task

Read the full document carefully. Then produce a structured evaluation response with exactly the five sections below. Do not add preamble, do not add pleasantries, do not hedge with "overall this is interesting." Go directly to substance.

### Section 1 — Prior art check

For each "novel/uncommon pattern" claimed in §6 of the document (items 6.1 through 6.9), classify it as one of:

- **GENUINELY NOVEL** — I cannot find prior art for this specific mechanism
- **UNCOMMON BUT EXISTS** — similar mechanisms exist in [cite]; the specific shape here is [same / different]
- **DERIVATIVE** — standard pattern widely implemented (cite 2-3 examples)

For UNCOMMON and DERIVATIVE classifications, give specific citations (paper titles, framework names, blog posts, GitHub repos). Do not fabricate citations. If you're uncertain, say "I'm uncertain — candidates are X, Y, Z, would need to verify."

### Section 2 — Attack surface analysis

If you were adversarially attacking this system to make it fail, which primitive would you attack first, and how specifically? Give a concrete attack sequence — prompts, inputs, or exploitation paths. Rank the top 3 vulnerabilities in order of exploitability.

### Section 3 — Ambient awareness + relationship layer assessment

Read §8 (ambient awareness) and §9 (relationship layer) carefully. These are proposed but not built. The author believes they may be the most genuinely novel parts of the project. Answer:

a) **Ambient awareness (§8):** Is the evidence-required + 14-day decay + opportunistic preamble design sound? What fails first? Any prior art you recognize?

b) **Relationship layer (§9):** Is second-order relational modeling ("Agent A knows something about B that matters for C") worth the primitive complexity at solo-operator scale? Any prior art in multi-agent systems specifically (not just theoretical theory-of-mind)?

c) Of the two, which would you prioritize building first? Why?

### Section 4 — Research direction ranking

Read §10 (eight research-worthy directions). Re-rank them from highest to lowest research value, with a one-sentence justification per position. Be honest — if any are intellectual theater rather than viable research, say so.

### Section 5 — What's missing

What primitive, architectural pattern, or design consideration is the author missing that you'd add? Keep suggestions specific and implementable, not aspirational.

## Style requirements

- No pleasantries. No "this is impressive." No "overall the project has merit."
- No hedging. If you think something is wrong, say it's wrong.
- Every claim about prior art must include a specific citation you'd stand behind. If you can't cite, flag it as uncertain.
- If any claim in the document is unsupported by the code shown, flag it.
- Use bullet points and structure. Prose walls are hard to extract value from.
- Target length: 1,500-3,000 words for the whole response. Shorter is acceptable if you can be precise.

## What NOT to do

- Do not suggest the author use LangGraph / CrewAI / AutoGen "instead." He's evaluated these and chose a different shape.
- Do not propose productizing the framework. He has explicitly cut that from scope.
- Do not give strategic business advice. The question is about the technical design.
- Do not summarize the document back to him. He wrote it.

---

# COMPANY OS — EVALUATION PACKAGE (full document begins below)
...
Expensive queries surface to founder before execution (not after). Self-improvement proposals expire in 7 days, forcing real decisions rather than drift into pending purgatory. Claim: small primitives, but the forcing-function property is rare.
...
# END OF COMPANY OS EVALUATION PACKAGE

Now respond with the five-section evaluation described at the top of this prompt. Start directly with "## Section 1 — Prior art check" — no preamble.
